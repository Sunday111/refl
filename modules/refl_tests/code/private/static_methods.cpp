...
TEST(reflTest, StaticMethod)
{
    ...
    {
        const refl::Function* f = methods[0];
        ...
        refl::CallFunction<void>(f);
        EXPECT_TRUE(ReflectedType::k == 11);
    }

    {
        const refl::Function* f = methods[1];
        EXPECT_TRUE(f != nullptr);
        EXPECT_TRUE(f->GetName() == std::string_view("f2"));
        EXPECT_TRUE(f->GetObjectType() == nullptr);
        EXPECT_TRUE(f->GetReturnType() == nullptr);
        EXPECT_TRUE(f->GetArguments().size() == 1);
        EXPECT_TRUE(f->GetArguments()[0] == refl::GetTypeInfo<int>());
        refl::CallFunction<void>(f, 2);
        EXPECT_TRUE(ReflectedType::k == 13);
    }

    {
        const refl::Function* f = methods[2];
        EXPECT_TRUE(f != nullptr);
        ...
        EXPECT_TRUE(f->GetArguments()[0] == refl::GetTypeInfo<int*>());
        int arg = 3;
        refl::CallFunction<void>(f, &arg);
        EXPECT_TRUE(arg == 114);
    }

    {
        const refl::Function* f = methods[3];
        EXPECT_TRUE(f != nullptr);
