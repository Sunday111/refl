...
{
    int value = 17;
    static void ReflectType(cppreflection::TypeReflector<OverAligned>& reflector) { reflector.SetName("OverAligned"); }
};
...
struct NonDefault
{
    ...
    int value;
    static void ReflectType(cppreflection::TypeReflector<NonDefault>& reflector) { reflector.SetName("NonDefault"); }
};
...
struct MoveOnly
{
    ...
    static void ReflectType(cppreflection::TypeReflector<MoveOnly>& reflector) { reflector.SetName("MoveOnly"); }
};

struct CopyableValue
{
    int value = 0;
    static void ReflectType(cppreflection::TypeReflector<CopyableValue>& reflector)
    {
...
struct ThrowOnSecondCopy
{
    ...
    static void ReflectType(cppreflection::TypeReflector<ThrowOnSecondCopy>& reflector)
    {
...
struct InvocationHost
{
    ...
    {
        return cppreflection::StaticClassTypeInfo<InvocationHost>(
                   "InvocationHost",
...
{
    const auto* type = cppreflection::GetTypeInfo<InvocationHost>();

    const auto over_aligned = cppreflection::CallFunction<OverAligned>(FindMethod(type, "MakeOverAligned"));
    EXPECT_EQ(over_aligned.value, 17);

    const auto non_default = cppreflection::CallFunction<NonDefault>(FindMethod(type, "MakeNonDefault"));
    EXPECT_EQ(non_default.value, 23);

    auto move_only = cppreflection::CallFunction<MoveOnly>(FindMethod(type, "MakeMoveOnly"));
    ASSERT_NE(move_only.value, nullptr);
    EXPECT_EQ(*move_only.value, 31);

    MoveOnly consumed(37);
    cppreflection::CallFunction<void>(FindMethod(type, "ConsumeMoveOnly"), std::move(consumed));
    EXPECT_EQ(consumed.value, nullptr);
    EXPECT_EQ(GetInvocationState().consumed_move_only, 37);

    CopyableValue copyable{38};
    cppreflection::CallFunction<void>(FindMethod(type, "ConsumeCopyable"), copyable);
    EXPECT_EQ(copyable.value, 38);
    EXPECT_EQ(GetInvocationState().consumed_copyable, 38);

    MoveOnly lvalue_move_only(40);
    EXPECT_THROW(
        cppreflection::CallFunction<void>(FindMethod(type, "ConsumeMoveOnly"), lvalue_move_only),
        std::invalid_argument);
    ASSERT_NE(lvalue_move_only.value, nullptr);

    const MoveOnly observed(39);
    cppreflection::CallFunction<void>(FindMethod(type, "ObserveMoveOnly"), observed);
    ASSERT_NE(observed.value, nullptr);
    EXPECT_EQ(GetInvocationState().observed_move_only, 39);

    auto& reference = cppreflection::CallFunction<int&>(FindMethod(type, "GetReference"));
    reference = 43;
    EXPECT_EQ(GetInvocationState().global_value, 43);

    auto moved_reference = cppreflection::CallFunction<MoveOnly&&>(FindMethod(type, "GetRvalueReference"));
    ...
    cppreflection::CallFunction<void>(FindMethod(type, "CallVoid"));
    EXPECT_EQ(GetInvocationState().void_calls, 1);
    EXPECT_THROW(
        static_cast<void>(cppreflection::CallFunction<int>(FindMethod(type, "ThrowingFunction"))),
        std::runtime_error);

    ThrowOnSecondCopy::Reset();
    EXPECT_THROW(
        static_cast<void>(cppreflection::CallFunction<ThrowOnSecondCopy>(FindMethod(type, "MakeThrowOnSecondCopy"))),
        std::runtime_error);
